    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
            <div
                ...
                onClick={onClose}
            />
            <div className="relative w-full max-w-4xl bg-white rounded-[2.5rem] shadow-2xl border border-gray-100 overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
                {/* Header */}
                <div className="p-8 pb-6 border-b border-gray-100 flex items-center justify-between bg-white sticky top-0 z-10">
                    <div className="flex items-center gap-5">
                        ...
                        <div>
                            <h2 className="text-2xl font-black text-gray-900 tracking-tight">Loan Portfolio Details</h2>
                            <div className="flex flex-col sm:flex-row sm:items-center gap-3 mt-1">
                                <div className="flex items-center gap-2">
                                    <span className="text-xs text-gray-400 font-bold uppercase tracking-widest">System ID: <span className="text-blue-600 font-black">{loan.contract_number || loan.loan_id}</span></span>
                                ...
                                {loan.contract_number && (
                                    <span className="text-[10px] text-gray-400 font-medium">Ref: {loan.loan_id}</span>
                                )}
                            </div>
                        </div>
                    </div>
                    <button
                        onClick={onClose}
                        className="w-12 h-12 flex items-center justify-center rounded-2xl bg-gray-50 text-gray-400 hover:bg-red-50 hover:text-red-500 transition-all active:scale-95"
                    >
...
function DetailItem({ label, value, subValue, highlight }: { label: string; value: React.ReactNode; subValue?: string; highlight?: boolean }) {
    return (
        <div className="group">
            ...
            <div className={`text-sm font-bold truncate ${highlight ? 'text-blue-600 text-base' : 'text-gray-900'}`}>
                {value || '-'}
            </div>
