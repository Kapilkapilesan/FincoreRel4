                    ...
                    <div className="p-5 bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-200 rounded-xl shadow-sm space-y-4">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-y-4 gap-x-6 text-sm">
                            ...
                            <div className="space-y-1">
                                ...
                                <p className="font-semibold text-gray-900">{(selectedCustomerRecord as any).gender || 'N/A'} ({(selectedCustomerRecord as any).age ?? 'N/A'} yrs)</p>
                            </div>
                            <div className="space-y-1">
                                <p className="text-[10px] text-blue-600 font-bold uppercase tracking-widest">Mobile Number</p>
                                <p className="font-semibold text-gray-900">{(selectedCustomerRecord as any).phone || 'N/A'}</p>
                            </div>
                            ...
                            <div className="md:col-span-2 p-3 bg-white/50 rounded-lg border border-blue-100/50">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <p className="font-bold text-green-700 tracking-tight">LKR {((selectedCustomerRecord as any).monthly_income || 0).toLocaleString()}</p>
                                    </div>
                                    <div>
                                        <p className="text-xs text-gray-500 italic">Loan Exposure</p>
                                        <p className="font-bold text-red-700 tracking-tight">LKR {((selectedCustomerRecord as any).activeLoanAmount || 0).toLocaleString()}</p>
                                    </div>
                                </div>
                            </div>

                            <div className="md:col-span-2 p-3 bg-white/50 rounded-lg border border-blue-100/50">
                                <p className="text-[10px] text-blue-600 font-bold uppercase tracking-widest mb-1.5">Reloan Verification</p>
                                <div className="space-y-2">
                                    <div className="flex justify-between items-center text-xs">
                                        <span className="text-gray-600">Paid Progress</span>
                                        <span className={`font-bold ${(selectedCustomerRecord as any).reloan_eligibility?.isEligible ? 'text-green-600' : 'text-orange-500'}`}>
                                            {(selectedCustomerRecord as any).reloan_eligibility?.progress || 0}%
                                        </span>
                                    </div>
                                    <div className="w-full bg-gray-200 rounded-full h-1.5">
                                        <div
                                            className={`h-1.5 rounded-full transition-all duration-500 ${(selectedCustomerRecord as any).reloan_eligibility?.isEligible ? 'bg-green-500' : 'bg-orange-400'}`}
                                            style={{ width: `${(selectedCustomerRecord as any).reloan_eligibility?.progress || 0}%` }}
                                        ></div>
                                    </div>
                                    <p className="text-[9px] text-gray-500 leading-none">
                                        {(selectedCustomerRecord as any).reloan_eligibility?.isEligible
                                            ? '✓ ELIGIBLE: Outstanding balance will be deducted from payout.'
                                            : '⚠ BLOCKED: 75% of current loan must be paid.'}
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="grid md:grid-cols-2 gap-4 bg-gray-50/50 p-5 rounded-xl border border-gray-100">
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full px-3 py-2 border border-gray-200 bg-white rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
                                    required
                                />
                            </div>
                            <div>
                        ...
                        <div className="grid md:grid-cols-2 gap-4 bg-gray-50/50 p-5 rounded-xl border border-gray-100">
                            <div>
                                ...
                                <input
                                    ...
                                    className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm transition-all ${formData.guardian_nic && !isValidNIC(formData.guardian_nic) ? 'border-red-500 bg-red-50' :
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm ${formData.guardian_phone && !/^\d{10}$/.test(formData.guardian_phone) ? 'border-red-500 bg-red-50' : 'border-gray-200 bg-white'}`}
                                    required
                                />
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <div className="relative group/select">
                                    <select
                                        ...
                                        className={`w-full pl-4 pr-10 py-3 bg-white border-2 rounded-xl focus:outline-none focus:ring-4 focus:ring-blue-500/10 transition-all text-sm font-semibold appearance-none cursor-pointer ${formData.witness2_id ? 'border-blue-100 text-blue-900 font-bold' : 'border-gray-100 text-gray-500'}`}
                                    >
