            ...
            if (customer) {
                ...
                customerService.getCustomer(customer.id).then(fullCustomer => {
                    if (fullCustomer) {
                        ...
                        const completedLoans = allLoans.filter((l: any) => l.status === 'Completed' || l.status === 'Active');
                        ...
                        setSelectedCustomerRecord(prev => {
                            const enriched: any = {
                                ...
                                // Calculate 75% eligibility for the most recent active loan
                                reloan_eligibility: activeLoans.length > 0 ? (() => {
                                    const latest = activeLoans[0];
                                    const totalAmount = Number(latest.approved_amount) + (Number(latest.approved_amount) * (Number(latest.interest_rate) / 100));
                                    const rental = totalAmount / latest.terms;
                                    const paid = totalAmount - Number(latest.outstanding_amount);
                                    const progress = (paid / rental) / latest.terms;
                                    return {
                                        isEligible: progress >= 0.749,
                                        progress: Math.min(100, Math.round(progress * 100)),
                                        balance: Number(latest.outstanding_amount)
                                    };
                                })() : null
                            };
